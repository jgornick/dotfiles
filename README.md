...
| `private_dot_Brewfile.tmpl` | `~/.Brewfile` |
...
- **`.chezmoiignore`** — keeps repo-only files (`README.md`, `AGENTS.md`,
  `setup.sh`, `.vscode/`, `prefs/`) out of `$HOME`.
...
| `~/.ssh/github.oss` | work machines only: the key `~/.ssh/config.oss` uses for `~/Projects/oss` repos; created by hand |
| `~/.ssh/known_hosts`, `~/.ssh/known_hosts.oss` | github.com host keys, written by `setup.sh` |
| `~/.npmrc` | written by `npm login`; holds auth tokens |
| `~/.config/chezmoi/chezmoi.toml` | points chezmoi at this repo and holds machine-local data such as `work_machine = true` |

On a work machine, set `[data] work_machine = true` in the local chezmoi
config. The rendered Brewfile then omits casks that cannot be installed there;
the shared template still includes them for personal machines. For a first-run
bootstrap before chezmoi exists, use `DOTFILES_WORK_MACHINE=1 ./setup.sh`.
...
Monosnap's save directory is machine-local and deliberately omitted from its
snapshot; import preserves the path already configured on each Mac.

The work-machine profile skips the Claude Usage snapshot because that app is
not used there; it remains tracked and imported on personal Macs.
...
`private_dot_Brewfile.tmpl` rather than downloading whatever is on `master`.
